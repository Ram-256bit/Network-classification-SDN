...
class LiveTrafficClassifier:
    def __init__(self):
        ...
        try:
            ...
            # Remove target columns from feature names for live prediction
            self.prediction_features = [f for f in self.feature_names if f not in ['attack_cat', 'label']]
            print(f"Using {len(self.prediction_features)} features for prediction")
    ...
    def preprocess_live_data(self, df):
        ...
        # Ensure all expected columns are present (only use prediction features)
        for col in self.prediction_features:
            if col not in df_processed.columns:
                df_processed[col] = 0
        
        # Reorder columns to match training data (only prediction features)
        available_features = [col for col in self.prediction_features if col in df_processed.columns]
