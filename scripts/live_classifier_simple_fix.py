...
class LiveTrafficClassifier:
    def __init__(self):
        ...
        try:
            ...
            # The actual features used for training (excluding target columns)
            self.training_features = [f for f in self.feature_names if f not in ['attack_cat', 'label']]
            print(f"Training used {len(self.training_features)} features")
    ...
    def preprocess_live_data(self, df):
        ...
        # Add missing training features with default values
        for col in self.training_features:
            if col not in df_processed.columns:
                df_processed[col] = 0
        
        # Reorder columns to match training data order
        df_processed = df_processed[self.training_features]
